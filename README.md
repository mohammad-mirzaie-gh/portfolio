...
The UI was redesigned for a more premium, senior-engineer feel while keeping
the original brand colors and server-first architecture intact:

- **New signature backdrop** — a quiet blueprint/schematic grid with slow
  drifting glow orbs (`components/backdrop/Backdrop.tsx`), pure CSS, respects
  `prefers-reduced-motion`.
- **Refined palette** — deeper background (`#050b14`), calibrated teal accent,
  and a sparingly-used warm amber for variety (see `--color-*` tokens in
  `app/globals.css`).
- **Sans + mono type pairing** — mono is used for "system" details (section
  eyebrows like `// 01 — about`, stat labels, nav indices) to reinforce the
  developer identity without being kitschy.
- **Hero** — added a live-status pill, a real stat strip, and a résumé
  download CTA alongside the contact CTA.
- **About/Skills** — skills are now grouped by category (Languages, Frontend,
  Backend, Data, Tooling) instead of one long unsorted badge list.
- **Projects** — the project card now carries tags, a status badge, and is
  built to extend cleanly as more projects are added (`PROJECTS` array in
  `components/section/MainSection.tsx`).
- **Contact** — channels (email/GitHub/LinkedIn/Telegram) are now labeled
  cards instead of a bare icon row.
- **No external font fetch** — swapped `next/font/google` for a curated
  native font stack, so `npm run build` never depends on reaching Google's
  servers.
